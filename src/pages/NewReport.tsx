...
import { fetchDiseases, fetchSymptoms, submitReport, submitReportSymptoms } from "../lib/queries";
...
export default function NewReport() {
  ...
  useEffect(() => {
    ...
    const loadMetadata = async () => {
      try {
        const [{ data: diseasesData, error: diseasesError }, { data: symptomsData, error: symptomsError }] =
          await Promise.all([fetchDiseases(), fetchSymptoms()]);

        if (diseasesError) handleError(diseasesError, { context: 'Fetch Diseases' });
        else setDiseases(diseasesData);

        if (symptomsError) handleError(symptomsError, { context: 'Fetch Symptoms' });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (reportData?.report_id) {
        const { error: sympError } = await submitReportSymptoms(reportData.report_id, selectedSymptoms);
      ...
      navigate("/report-success", { state: { reportId: reportData?.report_id } });
    } catch (err) {
      ...
      <main className={MAIN_CLASS}>
        <div className="p-4 space-y-6">
          <div className="bg-white dark:bg-surface-dark p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
            <label className="block text-sm font-bold text-text-muted mb-2 font-almarai">{t('Patient Name')}</label>
            <div className="relative">
              ...
              />
              <span className="material-symbols-outlined absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-gray-700">person</span>
            </div>
          </div>

          <div className="bg-white dark:bg-surface-dark p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
            <div className="flex justify-between items-center mb-3">
              <label className="text-sm font-bold text-text-muted font-almarai">{t('Set Location')}</label>
              <button 
            ...
            {location ? (
              <div className="p-3 bg-green-50 dark:bg-green-900/20 rounded-xl text-green-700 dark:text-green-400 text-sm font-bold text-center border border-green-200 dark:border-green-800 mb-3">
                {t('Location set successfully')} ✓ <br/><span className="text-xs font-normal">({location.lat.toFixed(4)}, {location.lng.toFixed(4)})</span>
              </div>
            ) : null}
            ...
            <div className="relative w-full h-48 bg-gray-200 dark:bg-gray-800 rounded-xl overflow-hidden shadow-inner z-0 border border-gray-200 dark:border-gray-800">
              <MapContainer center={[12.8, 45.0333]} zoom={12} style={{ height: '100%', width: '100%', zIndex: 1 }}>
          ...
          <div className="bg-white dark:bg-surface-dark p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 space-y-3">
            <label className="block text-sm font-bold text-text-muted mb-1 font-almarai">{t('Select Disease')}</label>
            <div className={`grid grid-cols-1 gap-3 max-h-[400px] overflow-y-auto ${i18n.language === 'ar' ? 'pr-1' : 'pl-1'} scrollbar-hide`} dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}>
              <label className="cursor-pointer block group">
                <input className="hidden" name="disease" type="radio" checked={selectedDisease === "unknown"} onChange={(e) => e.target.checked && setSelectedDisease("unknown")} />
                <div className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/5 transition-all group-hover:bg-gray-100 dark:group-hover:bg-white/10 group-has-[:checked]:!bg-primary group-has-[:checked]:!border-primary">
                  <h4 className="font-bold text-primary group-has-[:checked]:!text-white mb-1 font-almarai text-base">{t("I don't know (Rely on symptoms)")}</h4>
                  <p className="text-xs text-text-muted dark:text-gray-800 group-has-[:checked]:!text-white/90">{t('The system will auto-classify based on selected symptoms.')}</p>
                </div>
              </label>
              {diseases.map((disease) => (
                <label key={disease.disease_id} className="cursor-pointer block group">
                  <input className="hidden" name="disease" type="radio" checked={selectedDisease === disease.disease_id} onChange={(e) => e.target.checked && setSelectedDisease(disease.disease_id)} />
                  <div className="p-4 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/5 transition-all group-hover:bg-gray-100 dark:group-hover:bg-white/10 group-has-[:checked]:!bg-primary group-has-[:checked]:!border-primary">
                    <h4 className="font-bold text-primary group-has-[:checked]:!text-white mb-1 font-almarai text-base">
                      {pickLocalizedName(disease.disease_name, disease.ar_name, i18n.language)}
                    </h4>
                    {disease.description && <p className="text-xs text-text-muted dark:text-gray-800 group-has-[:checked]:!text-white/90">{disease.description}</p>}
                  </div>
                </label>
              ))}
            </div>
          ...
          <div className="bg-white dark:bg-surface-dark p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800">
            <label className="block text-sm font-bold text-text-muted mb-3 font-almarai">{t('Symptoms (Select all that apply)')}</label>
            <div className={`flex flex-wrap gap-2 max-h-[300px] overflow-y-auto scrollbar-hide ${i18n.language === 'ar' ? 'pr-1' : 'pl-1'}`} dir={i18n.language === 'ar' ? 'rtl' : 'ltr'}>
              {symptoms.map((symptom) => {
                const label = pickLocalizedName(symptom.symptom_name, symptom.ar_name, i18n.language);
                const checked = selectedSymptoms.includes(symptom.symptom_id);

                return (
                  <label key={symptom.symptom_id} className="cursor-pointer group">
                    <input
                      className="hidden"
                      type="checkbox"
                      checked={checked}
                      onChange={(e) => toggleSymptom(symptom.symptom_id, e.target.checked)}
                    />
                  <div className="flex items-center gap-2 px-3 py-2 rounded-full border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-white/5 text-text-muted dark:text-gray-800 transition-all select-none group-has-[:checked]:!bg-primary group-has-[:checked]:!text-white group-has-[:checked]:!border-primary">
                    <span className="text-sm font-medium font-almarai">{label}</span>
                  </div>
                  </label>
                );
              })}
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background-light/80 dark:bg-background-dark/80 backdrop-blur-md p-4">
          <div className="bg-white dark:bg-surface-dark w-full max-w-sm rounded-[2rem] p-8 shadow-2xl text-center animate-in fade-in zoom-in-95 duration-300">
            ...
            <h2 className="text-2xl font-extrabold text-foreground dark:text-white mb-3 tracking-tight">{t("Login Required")}</h2>
            <p className="text-muted-foreground dark:text-gray-400 mb-8 leading-relaxed font-medium">{t("Sorry, you must log in to your account first in order to contribute and submit a new health report.")}</p>
            <div className="flex flex-col gap-3">
              <button onClick={() => navigate("/login")} className="w-full bg-primary hover:bg-primary-dark text-white text-lg font-bold py-4 rounded-2xl transition-all duration-300 shadow-lg active:scale-[0.98]">{t("Login Now")}</button>
              <button onClick={() => navigate("/")} className="w-full bg-muted dark:bg-white/5 text-muted-foreground dark:text-gray-300 font-bold py-4 rounded-2xl transition-all duration-300 active:scale-[0.98]">{t("Back to Home")}</button>
            </div>
          </div>
        </div>
      )}
